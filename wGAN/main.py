...
fh5py = h5py.File('struc.h5','r')
a = fh5py['data'][:]
fh5py.close()
print(type(a),a.shape)
(nn1, nn2, nn3, nn4, nn5) = a.shape
nsample = nn1/opt.batchSize
print(nsample)
aa = a[0:nsample*opt.batchSize,:,:,:]
ab = aa.reshape([nsample, opt.batchSize, nn2, nn3, nn4, nn5])
#print(len(ab))
torch_a = torch.from_numpy(ab).float()
#---------------------------------------------------------------------
for epoch in range(opt.niter):
    #data_iter = iter(dataloader)
    #print(type(data_iter),torch_a.size(0))
    i = 0
    #while i < torch_a.size(0):
    while i < torch_a.size(0):
        ############################
        # (1) Update D network
        ###########################
        ...
        if gen_iterations < 25 or gen_iterations % 500 == 0:
            ...
            Diters = opt.Diters
        j = 0
        while j < Diters and i <  torch_a.size(0):
            j += 1
        ...
        print('[%d/%d][%d/%d][%d] Loss_D: %f Loss_G: %f Loss_D_real: %f Loss_D_fake %f'
            % (epoch, opt.niter, i, torch_a.size(0), gen_iterations,
